...
const webpackConfig = {
  ...
  output: { path: path.resolve(__dirname, "./build/assets"), filename: "build.js" },
...
let static = gulp.parallel(
  ...
  () => gulp.src("./node_modules/@fortawesome/fontawesome-free/webfonts/*.ttf").pipe(gulp.dest("./build/assets/fonts")),
  () => gulp.src("./src/images/**/*").pipe(gulp.dest("./build/assets/images")),
  () => gulp.src("./src/*.html").pipe(gulp.dest("./build")),
  () => gulp.src("./src/.well-known/**/*").pipe(gulp.dest("./build/.well-known"))
);
...
module.exports = {
  ...
  start: gulp.series(clean, build, start)
};
